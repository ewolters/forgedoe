"""Conjugate posterior routines for the Normal-Inverse-Gamma linear model.

All routines operate in precision form internally (Lambda_n = V_n^{-1}).
Covariance is computed only when needed for reporting.

Math:
    y = X*beta + eps,  eps ~ N(0, sigma^2 * I)
    Prior: beta|sigma^2 ~ N(mu_0, sigma^2 * Lambda_0^{-1}),
           sigma^2 ~ InvGamma(alpha_0, beta_0)
    Posterior: beta|sigma^2,y ~ N(mu_n, sigma^2 * Lambda_n^{-1}),
              sigma^2|y ~ InvGamma(alpha_n, beta_n)

    where  Lambda_n = Lambda_0 + X'X
           mu_n = Lambda_n^{-1}(Lambda_0 * mu_0 + X'y)
           alpha_n = alpha_0 + n/2
           beta_n = beta_0 + 0.5*(y'y + mu_0'Lambda_0*mu_0 - mu_n'Lambda_n*mu_n)

    Marginal: c'beta | y ~ t(2*alpha_n, c'mu_n, sqrt(beta_n/alpha_n * c'Lambda_n^{-1}c))
...
def bayesian_linear_posterior(X, y, mu0=None, Lambda0=None, alpha0=2.0, beta0=None):
    ...
    alpha0 : float
        Prior shape for sigma^2.  Default: 2.0 (finite variance).
    beta0 : float, optional
        Prior scale for sigma^2.  Default: alpha0 * s^2 (centered on sample variance).
...
def contrast_posterior(c, mu_n, Lambda_n, L_n, alpha_n, beta_n):
    """Marginal posterior of a linear contrast c'beta.

    c'beta | y ~ Student-t(df=2*alpha_n, loc=c'mu_n,
                           scale=sqrt(beta_n/alpha_n * c'Lambda_n^{-1}c))
...
def predictive_posterior(x_new, mu_n, Lambda_n, L_n, alpha_n, beta_n):
    ...
    y*|x*,y ~ Student-t(df=2*alpha_n, loc=x*'mu_n,
                        scale=sqrt(beta_n/alpha_n * (1 + x*'Lambda_n^{-1}x*)))
